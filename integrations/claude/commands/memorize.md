...
Paths: the plugin ships the tooling under `${CLAUDE_PLUGIN_ROOT}` (repo root two
levels up); your private MEMORIES live at `$MEM_HOME` (default `~/workplace/mymemories`).

Steps:
1. Determine the current project's partition: run `readlink ~/.claude/projects/<mangled-cwd>/memory` — if it resolves into `$MEM_HOME/<partition>`, that's the target dir. If it is NOT linked yet, tell the user and offer to run `python3 <repo>/mymem install`.
2. Read `<repo>/format.md` and follow it EXACTLY: lead with the fact, commands verbatim, drop narration/dates/session-ids.
